...
class MySQLValueRepositoryTest(
    ...
    companion object {
        ...
        val container = DockerComposeContainer(File("src/test/resources/db/migration/docker-compose.yaml"))
        ...
        fun registerDynamicProperties(registry: DynamicPropertyRegistry) {
            registry.add("spring.r2dbc.url") { "r2dbc:mysql://${container.getServiceHost("mysql-test", dockerServicePort)}:${container.getServicePort(dockerServiceName, dockerServicePort)}/$databaseName" }
    ...
    @Test
    fun `저장 되어있는 데이터의 key값으로 상세조회 요청 시, 생성된 데이터값과 일치하는 데이터가 조회 되어야 한다`() = runBlocking {
    ...
    @Test
    fun `저장 되어있지 않은 객체 조회 시, null값이 반환 되어야 한다`() = runBlocking {
    ...
    @Test
    fun `저장 되어있는 key값을 다른 value로 upsert 요청 시, 다른 value로 update 되어야 한다`() = runBlocking {
